...
REGISTERED_TOOLS = set(ACTION_POLICY.keys())
...
def evaluate_tool_request(tool_request: ToolRequest) -> PolicyOutcome:
    ...
    if mode == "auto":
        return PolicyOutcome(kind="allow", reason="safe informational action", blocking_code=None, advisory_notes=[])
    if mode == "confirm":
        return PolicyOutcome(kind="require_confirmation", reason="modifying actions require approval", blocking_code=None, advisory_notes=["Await explicit user confirmation before execution."])
    return PolicyOutcome(kind="block", reason=f"No policy defined for tool: {tool_request.tool_name}", blocking_code="MISSING_POLICY", advisory_notes=[])


def evaluate_requested_action(action: RequestedAction) -> PolicyOutcome:
    return evaluate_tool_request(ToolRequest(
